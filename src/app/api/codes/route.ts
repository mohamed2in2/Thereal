...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!rateCheck.allowed) {
      await AccessCodeGuard.logAttempt({ ip: clientIp, userId: session.id, codeAttempted: codeStr, success: false });
      return NextResponse.json(
    ...
    if (moneyCode) {
      ...
      try {
        ...
        return NextResponse.json({ error: "تعذر معالجة الكود — يرجى المحاولة مرة أخرى" }, { status: 400 });
      }
    }
      ...
      if (purchaseType && targetId) {
        ...
        if (finalPrice === 0 || userBalance >= finalPrice) {
          ...
          if (purchaseResult?.success) {
            await AccessCodeGuard.logAttempt({ ip: clientIp, userId: session.id, codeAttempted: normalizedCode, success: true });
            return NextResponse.json({ success: true, type: "discount_purchase", message: purchaseResult.message || "تم تطبيق الخصم وتفعيل المحتوى بنجاح! 🎉" });
          } else {
